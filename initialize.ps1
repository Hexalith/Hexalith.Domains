...
Write-Verbose "Replacing 'Domains' with '$PackageName' in all files..."
Write-Verbose "Also replacing 'Domains' with '$($PackageName.ToLower())' in all files..."
...
function ProcessFile {
    ...
    try {
        ...
        if ($content -match "Domains") {
            $content = $content -replace "Domains", $PackageName
            $hasChanges = $true
        }
        
        if ($content -match "Domains") {
            $content = $content -replace "Domains", $PackageName.ToLower()
            $hasChanges = $true
        }
...
Rename-ProjectItems -SearchPattern "Domains" -Replacement $PackageName -ItemType Directory
Rename-ProjectItems -SearchPattern "Domains" -Replacement $PackageName -ItemType File
Rename-ProjectItems -SearchPattern "Domains" -Replacement $PackageName.ToLower() -ItemType Directory
Rename-ProjectItems -SearchPattern "Domains" -Replacement $PackageName.ToLower() -ItemType File
...
Write-Output "Package name has been changed from 'Domains' to '$PackageName'"
Write-Output "Lowercase 'Domains' has been changed to '$($PackageName.ToLower())'" 